...
description: Fallback behavior when leslie-image-gen lacks OpenAI API credentials but Codex/native image generation is available
---

# Codex Image2 Fallback

When using `leslie-image-gen` with `--provider openai --model gpt-image-2`, the CLI can fail with:
...
1. Try `leslie-image-gen` when provider credentials are available.
...
3. Prefer a Codex/native raster backend in this order:
   ...
   - `leslie-image-gen --provider codex-cli` (preferred — wraps the bundled `scripts/codex-imagegen/main.ts`; the underlying repo-level package lives at `packages/leslie-codex-imagegen/src/main.ts` for standalone callers), if `codex` CLI is installed/logged in.
...
Avoid implying that `leslie-image-gen --provider openai` can use Codex OAuth without a dedicated provider implementation.
