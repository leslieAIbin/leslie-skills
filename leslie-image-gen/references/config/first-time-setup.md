...
description: First-time setup and default model selection flow for leslie-image-gen
---
...
| Choice | Path | Scope |
|--------|------|-------|
| Project | `.leslie-skills/leslie-image-gen/EXTEND.md` | Current project |
| User | `$HOME/.leslie-skills/leslie-image-gen/EXTEND.md` | All projects |
...
- In `leslie-image-gen`, Azure `--model` / `default_model.azure` should be the Azure deployment name, not just the underlying model family.
...
- In `leslie-image-gen`, `quality` is a compatibility preset. It is not a native DashScope parameter.
...
- In `leslie-image-gen`, Z.AI currently exposes text-to-image only; reference images are not wired for this provider.
...
options:
  - label: "google/nano-banana-2 (Recommended)"
    description: "Current default for general Replicate image generation in leslie-image-gen"
