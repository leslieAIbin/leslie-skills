---
name: leslie-image-gen
description: ZenMux-backed AI image generation using OpenAI GPT Image 2 (default) or Google Gemini 3.1 Flash Image. Supports text-to-image, reference images, aspect ratios, and batch generation from saved prompt files. Reads the API key from ZENMUX_API_KEY and keeps provider selection explicit and reproducible. Use when user asks to generate, create, or draw images.
---

# Image Generation (ZenMux)

This customized copy routes image generation through ZenMux. It supports two documented models: `openai/gpt-image-2` (default) and `google/gemini-3.1-flash-image`. Upstream provider adapters remain in the source tree for compatibility, but this copy does not select or configure them.

## ZenMux setup

Set the key in the shell or in the usual local `.leslie-skills/.env` file. Never commit the key or place it in `EXTEND.md`:

```bash
export ZENMUX_API_KEY="<your-rotated-zenmux-key>"
```

The adapter automatically uses `https://zenmux.ai/api/v1` for the OpenAI-compatible image endpoint and `https://zenmux.ai/api/vertex-ai` for the Google Vertex AI-compatible endpoint. The Google route adds `/v1/publishers/google/models/<model>:generateContent` and uses Bearer authentication. These base URLs can be overridden with `ZENMUX_OPENAI_BASE_URL` and `ZENMUX_GOOGLE_BASE_URL` when needed. See `references/providers/zenmux.md` for the request mapping.

Stable values are pinned in the Skill: the two ZenMux base URLs and model IDs. Secrets and machine-specific network settings are not pinned: `ZENMUX_API_KEY` and standard proxy variables are read from the current environment. Never print, commit, or copy the key into prompts, config, logs, or generated files. ZenMux's internal upstream provider routing remains automatic.
...
| Path | Scope |
|------|-------|
| `.leslie-skills/leslie-image-gen/EXTEND.md` | Project |
| `${XDG_CONFIG_HOME:-$HOME/.config}/leslie-skills/leslie-image-gen/EXTEND.md` | XDG |
| `$HOME/.leslie-skills/leslie-image-gen/EXTEND.md` | User home |
...
Legacy compatibility: if `.leslie-skills/leslie-imagine/EXTEND.md` exists and the new path doesn't, the runtime renames it to `leslie-image-gen`.
...
# OpenAI GPT Image 2
${BUN_X} {baseDir}/scripts/main.ts --prompt "A cat" --image out.png --provider openai --model openai/gpt-image-2

# ZenMux Google image model
${BUN_X} {baseDir}/scripts/main.ts --prompt "A cat" --image out.png --provider google --model google/gemini-3.1-flash-image
...
# Build a batch file from outline.md + prompts/ (e.g. leslie-article-illustrator output)
...
| `--provider google\|openai` | Force the ZenMux model family (auto-detect by default) |
...
| `ZENMUX_API_KEY` | ZenMux API key; enables both documented ZenMux image families |
...
| `ZENMUX_OPENAI_BASE_URL` | ZenMux OpenAI-compatible endpoint override (default `https://zenmux.ai/api/v1`) |
| `ZENMUX_GOOGLE_BASE_URL` | ZenMux Google-compatible endpoint override (default `https://zenmux.ai/api/vertex-ai`) |
...
| `LESLIE_IMAGE_GEN_MAX_WORKERS` | Override batch worker cap |
| `LESLIE_IMAGE_GEN_<PROVIDER>_CONCURRENCY` | Per-provider concurrency (e.g., `LESLIE_IMAGE_GEN_REPLICATE_CONCURRENCY`; for codex-cli use `LESLIE_IMAGE_GEN_CODEX_CLI_CONCURRENCY`) |
| `LESLIE_IMAGE_GEN_<PROVIDER>_START_INTERVAL_MS` | Per-provider start-gap |
| `LESLIE_CODEX_IMAGEGEN_BIN` | Override the codex-imagegen wrapper path for the `codex-cli` provider (default: bundled `scripts/codex-imagegen/main.ts`; accepts `.ts` or legacy `.sh`/binary) |
| `LESLIE_CODEX_IMAGEGEN_CACHE_DIR` | Enable idempotency cache for the `codex-cli` provider (off by default) |
| `LESLIE_CODEX_IMAGEGEN_TIMEOUT_MS` | Per-attempt `codex exec` timeout for the `codex-cli` provider (default: 300000 ms) |
| `LESLIE_CODEX_IMAGEGEN_RETRIES` | Wrapper-side retry attempts on retryable errors for the `codex-cli` provider (default: 2) |
| `LESLIE_CODEX_IMAGEGEN_LOG_FILE` | Append JSONL diagnostic log for the `codex-cli` provider |

**Load priority**: CLI args > EXTEND.md > env vars > `<cwd>/.leslie-skills/.env` > `~/.leslie-skills/.env`

### Codex/ChatGPT OAuth is not an OpenAI API key

`--provider openai --model openai/gpt-image-2` uses ZenMux's OpenAI-compatible Images API and requires `ZENMUX_API_KEY` in this customized copy. A Codex or ChatGPT desktop login is a different entitlement and is not a drop-in replacement for an API key.

The upstream Codex-native instructions are not part of this ZenMux-only configuration. If a Codex subscription is desired, use the native `imagegen` skill instead of this skill.

Do not put a Codex OAuth token into `ZENMUX_API_KEY`.
...
For this customized copy, the OpenAI default is `openai/gpt-image-2`; pass the full ZenMux model id with `--model` or set `OPENAI_IMAGE_MODEL`.
...
1. `--ref` provided + no `--provider` → auto-select Google → OpenAI (the two ZenMux-backed families)
...
4. Multiple keys → default priority: Google → OpenAI; the checked-in EXTEND.md pins OpenAI as the stable default
...
| Input is `outline.md` + `prompts/` (e.g. from `leslie-article-illustrator`) | Batch — use `{baseDir}/scripts/build-batch.ts` to assemble the payload | The outline + prompt files already contain everything needed |
...
| `references/codex-oauth-vs-openai-api-key.md` | Why Codex/ChatGPT OAuth image2 entitlement is not usable through leslie-image-gen's standard OpenAI API-key provider |
