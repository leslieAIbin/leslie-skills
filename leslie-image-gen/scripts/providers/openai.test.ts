...
test("OpenAI aspect-ratio parsing and size selection match model families", (t) => {
  useEnv(t, { ZENMUX_API_KEY: null, OPENAI_IMAGE_MODEL: null });
...
test("OpenAI defaults to the full ZenMux model ID when ZenMux is configured", (t) => {
  useEnv(t, {
    ZENMUX_API_KEY: `sk-ai-v1-${"a".repeat(64)}`,
    OPENAI_IMAGE_MODEL: null,
  });
  assert.equal(getDefaultModel(), "openai/gpt-image-2");
});

test("OpenAI ZenMux route prefers ZENMUX_API_KEY over OPENAI_API_KEY", async (t) => {
  const zenMuxKey = `sk-ai-v1-${"a".repeat(64)}`;
  useEnv(t, {
    ZENMUX_API_KEY: zenMuxKey,
    OPENAI_API_KEY: "official-openai-key",
    OPENAI_BASE_URL: null,
    ZENMUX_OPENAI_BASE_URL: null,
  });
  ...
  globalThis.fetch = async (input, init) => {
    assert.equal(String(input), "https://zenmux.ai/api/v1/images/generations");
    assert.equal(
      new Headers(init?.headers).get("authorization"),
      `Bearer ${zenMuxKey}`,
    );
    return Response.json({
      data: [{ b64_json: Buffer.from("zenmux-openai").toString("base64") }],
    });
  };
  ...
  assert.equal(Buffer.from(bytes).toString("utf8"), "zenmux-openai");
});
