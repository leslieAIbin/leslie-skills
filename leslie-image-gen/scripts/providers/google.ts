...
function getGoogleApiKey(): string | null {
  if (isZenMuxGoogleEndpoint()) return getZenMuxApiKey();
...
function getGoogleBaseUrl(): string {
  const zenmuxDefault =
    process.env.ZENMUX_GOOGLE_BASE_URL || ZENMUX_GOOGLE_BASE_URL;
  const base =
...
function isZenMuxGoogleEndpoint(): boolean {
  return isZenMuxUrl(getGoogleBaseUrl());
}

export function buildGoogleUrl(pathname: string): string {
  const base = getGoogleBaseUrl();
  const cleanedPath = pathname.replace(/^\/+/g, "");
  if (isZenMuxGoogleEndpoint()) {
    const match = cleanedPath.match(
      /^models\/(?:google\/)?(.+?)(?::(generateContent|predict))$/,
    );
    if (match) {
      return `${base}/v1/publishers/google/models/${match[1]}:${match[2]}`;
    }
...
async function postGoogleJsonViaFetch<T>(
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `Google API error (${res.status}): ${err}${requestIdSuffix(res)}`,
    );
  }
...
async function postGoogleJson<T>(pathname: string, body: unknown): Promise<T> {
  ...
  if (!apiKey) {
    throw new Error(
      isZenMuxGoogleEndpoint()
        ? "ZENMUX_API_KEY is required for the configured ZenMux Google endpoint"
...
async function generateWithGemini(
  ...
  const response = await postGoogleJson<{
    ...
    generationConfig: {
      responseModalities: isZenMuxGoogleEndpoint()
        ? ["TEXT", "IMAGE"]
        : ["IMAGE"],
      imageConfig,
    },
