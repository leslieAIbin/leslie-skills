...
export async function generateImage(
  ...
  const baseURL =
    ...
    (process.env.ZENMUX_API_KEY
      ? process.env.ZENMUX_OPENAI_BASE_URL || ZENMUX_OPENAI_BASE_URL
      : "https://api.openai.com/v1");
  const useZenMux = isZenMuxUrl(baseURL);
  const apiKey = useZenMux
    ? getZenMuxApiKey()
    : process.env.OPENAI_API_KEY || getZenMuxApiKey();

  if (!apiKey) {
    throw new Error(
      useZenMux
        ? "ZENMUX_API_KEY is required for the configured ZenMux OpenAI Images endpoint."
        : "OPENAI_API_KEY or ZENMUX_API_KEY is required. Codex/ChatGPT desktop login does not automatically grant Images API access to this script."
    );
  }
...
async function generateWithChatCompletions(
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `OpenAI API error (${res.status}): ${err}${requestIdSuffix(res)}`,
    );
  }
...
async function generateWithOpenAIEdits(
  ...
  if (!res.ok) {
    const err = await res.text();
    throw new Error(
      `OpenAI edits API error (${res.status}): ${err}${requestIdSuffix(res)}`,
    );
  }
