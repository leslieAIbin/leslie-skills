...
Environment variables:
  ...
  ZENMUX_API_KEY            ZenMux API key (enables the configured OpenAI/Google routes)
  ...
  LESLIE_IMAGE_GEN_<PROVIDER>_CONCURRENCY  Override provider concurrency (use underscores: LESLIE_IMAGE_GEN_CODEX_CLI_CONCURRENCY)
  LESLIE_IMAGE_GEN_<PROVIDER>_START_INTERVAL_MS  Override provider start gap in ms
  LESLIE_CODEX_IMAGEGEN_BIN  Path to codex-imagegen wrapper (default: bundled scripts/codex-imagegen/main.ts; accepts .ts or legacy .sh/binary)
  LESLIE_CODEX_IMAGEGEN_CACHE_DIR  Enable idempotency cache for codex-cli provider (default: disabled)
  LESLIE_CODEX_IMAGEGEN_TIMEOUT_MS  Per-attempt codex exec timeout for codex-cli provider (default: 300000)
  LESLIE_CODEX_IMAGEGEN_RETRIES  Codex-side retry attempts on retryable errors (default: 2)
  LESLIE_CODEX_IMAGEGEN_LOG_FILE  Append JSONL diagnostic log for codex-cli provider

Env file load order: CLI args > EXTEND.md > process.env > <cwd>/.leslie-skills/.env > ~/.leslie-skills/.env`);
}
...
async function loadEnv(): Promise<void> {
  ...
  const homeEnv = await loadEnvFile(path.join(home, ".leslie-skills", ".env"));
  const cwdEnv = await loadEnvFile(path.join(cwd, ".leslie-skills", ".env"));
...
function getExtendConfigPathPairs(cwd: string, home: string): ExtendConfigPathPair[] {
  return [
    {
      current: path.join(cwd, ".leslie-skills", "leslie-image-gen", "EXTEND.md"),
      legacy: path.join(cwd, ".leslie-skills", "leslie-imagine", "EXTEND.md"),
    },
    {
      current: path.join(home, ".leslie-skills", "leslie-image-gen", "EXTEND.md"),
      legacy: path.join(home, ".leslie-skills", "leslie-imagine", "EXTEND.md"),
    },
  ];
}
...
export function getConfiguredMaxWorkers(extendConfig: Partial<ExtendConfig>): number {
  const envValue = parsePositiveInt(process.env.LESLIE_IMAGE_GEN_MAX_WORKERS);
...
export function getConfiguredProviderRateLimits(
  ...
  for (const provider of ["replicate", "google", "openai", "openrouter", "dashscope", "zai", "minimax", "jimeng", "seedream", "azure", "codex-cli", "agnes"] as Provider[]) {
    const envPrefix = `LESLIE_IMAGE_GEN_${provider.toUpperCase().replace(/-/g, "_")}`;
    ...
    configured[provider] = {
      ...
      startIntervalMs:
        parsePositiveInt(
          process.env[`${envPrefix}_START_INTERVAL_MS`],
        ) ??
...
export function detectProvider(args: CliArgs): Provider {
  ...
  const hasOpenai = !!(process.env.OPENAI_API_KEY || process.env.ZENMUX_API_KEY);
  ...
  if (args.referenceImages.length > 0) {
    ...
    throw new Error(
      "Reference images require Google or OpenAI-compatible access. Set GOOGLE_API_KEY/GEMINI_API_KEY/ZENMUX_API_KEY, OPENAI_API_KEY, or another supported provider key, or remove --ref."
    );
  }
  ...
  throw new Error(
    "No API key found. Set ZENMUX_API_KEY (recommended for this customized copy), GOOGLE_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, or another supported provider key.\n" +
      "Create ~/.leslie-skills/.env or <cwd>/.leslie-skills/.env with your keys."
  );
}
...
export function isRetryableGenerationError(error: unknown): boolean {
  ...
  if (/\baccess_denied\b/i.test(msg)) return false;
  if (/\b(?:invalid|expired|missing)\b.{0,24}\b(?:api[ _-]?key|credential)/i.test(msg)) {
    return false;
  }
  if (/(?:API|HTTP)(?: edits)? error \((?:400|401|402|403|404|413|422)\)/i.test(msg)) {
    return false;
  }
