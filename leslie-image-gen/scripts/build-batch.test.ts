...
const scriptPath = path.join(scriptsDir, "build-batch.ts");
...
async function makeFixture(): Promise<{
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "leslie-image-gen-build-batch-"));
...
test("build-batch omits default model so leslie-image-gen can resolve env or EXTEND defaults", async () => {
