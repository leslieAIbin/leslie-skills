...
2. `<cwd>/.leslie-skills/.env` with `WECHAT_APP_ID=...`
3. `$HOME/.leslie-skills/.env` with `WECHAT_APP_ID=...`
...
Where to save?
A) Project-level: .leslie-skills/.env (this project only)
B) User-level: ~/.leslie-skills/.env (all projects)
```
