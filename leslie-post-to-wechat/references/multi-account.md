...
3. `.leslie-skills/.env` with the prefixed key `WECHAT_{ALIAS}_APP_ID`
4. `~/.leslie-skills/.env` with the prefixed key
...
```bash
# Account: leslie
WECHAT_LESLIE_APP_ID=your_wechat_app_id
WECHAT_LESLIE_APP_SECRET=your_wechat_app_secret
...
${BUN_X} {baseDir}/scripts/wechat-article.ts --markdown <file> --theme default --account leslie
${BUN_X} {baseDir}/scripts/wechat-browser.ts --markdown <file> --images ./photos/ --account leslie
```
...
${BUN_X} {baseDir}/scripts/wechat-api.ts <file> --theme default --account leslie --remote --remote-host other-server.example.com
```
