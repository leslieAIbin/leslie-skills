...
async function checkApiCredentials(): Promise<void> {
  ...
  const projectEnv = path.join(cwd, '.leslie-skills', '.env');
  const userEnv = path.join(os.homedir(), '.leslie-skills', '.env');
...
async function main(): Promise<void> {
  console.log('=== leslie-post-to-wechat: Permission & Environment Check ===\n');
