---
name: leslie-post-to-wechat
description: Prepares and optionally saves content as a WeChat Official Account draft via API or Chrome CDP. This customized copy never publishes publicly. Supports article and image-text draft preparation with HTML, Markdown, plain text, and multiple images. Use when user asks for a WeChat draft, 公众号草稿, or draft preview.
---

# Prepare WeChat Draft (no public publish)

This customized copy is draft-only. It may render local preview files and, when the user explicitly asks, save a draft through WeChat's draft endpoint. It must never publish an article publicly, click a public publish button, or call a public-publish endpoint.

### Safety boundary

- Default to local preview and rendered HTML.
- `--submit` means **save to the WeChat draft box only**, not publish; use it only after the user explicitly asks to save a draft.
- Never click `发布`, `Publish`, `Post`, or any externally visible submit control.
- Do not store AppSecret, access tokens, or other account credentials in this Skill directory or in `EXTEND.md`.
...
| Path | Scope |
|------|-------|
| `.leslie-skills/leslie-post-to-wechat/EXTEND.md` | Project |
| `${XDG_CONFIG_HOME:-$HOME/.config}/leslie-skills/leslie-post-to-wechat/EXTEND.md` | XDG |
| `$HOME/.leslie-skills/leslie-post-to-wechat/EXTEND.md` | User home |
...
| Profile dir | Shared profile at `leslie-skills/chrome-profile` |
...
| API credentials | Follow guided setup in Step 2, or set in `.leslie-skills/.env` |
...
${BUN_X} {baseDir}/scripts/wechat-browser.ts --title "标题" --content "内容" --image img.png --submit  # save draft only
```
...
- [ ] Step 4: Preview or save a WeChat draft
- [ ] Step 5: Report draft completion
```
...
### Step 2: Select Draft Method and Configure
...
**API selected + missing credentials** → run guided setup per `references/api-setup.md` (writes to `.leslie-skills/.env`).
...
**API draft method** (accepts `.md` or `.html`; calls `draft/add` only):
...
**Remote API draft method** (same script, adds `--remote`; still calls `draft/add` only):
...
**Browser preview/draft method** (accepts `--markdown` or `--html`; stop before any public publish control):
