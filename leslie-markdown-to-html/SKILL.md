...
This customized copy defaults to the `grace` theme with neutral Mermaid diagrams and is intended to feed the draft-only WeChat workflow. It performs local conversion and does not publish or call ZenMux.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.leslie-skills/leslie-markdown-to-html/EXTEND.md` | Project |
| 2 | `${XDG_CONFIG_HOME:-$HOME/.config}/leslie-skills/leslie-markdown-to-html/EXTEND.md` | XDG |
| 3 | `$HOME/.leslie-skills/leslie-markdown-to-html/EXTEND.md` | User home |
...
If CJK content detected AND `leslie-format-markdown` skill is available:
...
**If user agrees**: Invoke `leslie-format-markdown` skill to format the file, then use formatted file as input.
...
3. `leslie-post-to-wechat` EXTEND.md `default_theme` (cross-skill fallback)
...
Read `$HOME/.leslie-skills/leslie-post-to-wechat/EXTEND.md` if it exists and look for a `default_theme:` line. Use the value if present; otherwise fall through.
