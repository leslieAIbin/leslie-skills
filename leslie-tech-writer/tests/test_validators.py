...
def write_valid_package(project: Path) -> None:
    (project / "imgs").mkdir(parents=True)
    ...
    (project / "visual-plan.md").write_text(
        ...
        "|---|---|---|---|---|---|---|---|---|---|\n"
        "| V01 | imgs/01-cache.png | 机制 | 缓存什么 | 缓存避免重复计算 | E01; KV Cache | structural breakdown | leslie-infographic | accepted | PASS |\n",
        encoding="utf-8",
    )
    (project / "article.md").write_text(
        "# KV Cache 到底在缓存什么\n\nKV Cache 用空间换取解码阶段的重复计算。\n\n"
        "## 缓存边界\n\n![缓存结构](imgs/01-cache.png)\n\n证据见 E01。\n",
        encoding="utf-8",
    )
    (project / "qa-report.md").write_text(
    ...
    (project / "article.html").write_text("<h1>KV Cache 到底在缓存什么</h1>", encoding="utf-8")
    write_png(project / "imgs" / "01-cache.png")
...
class ValidatorTests(unittest.TestCase):
    ...
    def test_initialized_skeleton_fails_until_completed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            created = run(str(INIT), str(project), "--title", "测试文章")
    ...
    def test_release_rejects_pending_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            project = Path(tmp) / "article"
            write_valid_package(project)
            evidence = (project / "evidence.md").read_text(encoding="utf-8").replace("verified", "pending")
    ...
    def test_release_rejects_unaccepted_or_wrong_aspect_visual(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            project = Path(tmp) / "article"
            write_valid_package(project)
            plan = (project / "visual-plan.md").read_text(encoding="utf-8").replace("accepted | PASS", "regenerate | FAIL")
            (project / "visual-plan.md").write_text(plan, encoding="utf-8")
            write_png(project / "imgs" / "01-cache.png", 100, 100)
            result = run(str(VALIDATE), str(project), "--stage", "release")
            self.assertEqual(result.returncode, 1)
    ...
    def test_release_rejects_failed_qualitative_gate(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            project = Path(tmp) / "article"
            write_valid_package(project)
            qa = (project / "qa-report.md").read_text(encoding="utf-8").replace("Status: PASS", "Status: FAIL", 1)
            (project / "qa-report.md").write_text(qa, encoding="utf-8")
            result = run(str(VALIDATE), str(project), "--stage", "release")
            self.assertEqual(result.returncode, 1)
