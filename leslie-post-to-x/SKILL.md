---
name: leslie-post-to-x
description: Prepares X (Twitter) posts and long-form article drafts in Chrome, but never clicks Post or Publish. Use when the user asks to draft, preview, or prepare content for X without sending it.
---

# Prepare X Draft (no public post)

Prepares text, images, videos, and long-form articles for X in local files or the browser editor, but stops before any externally visible submission. This customized copy is draft/preview-only.

### Safety boundary

- Never click `Post`, `Publish`, `Schedule`, or any externally visible submit action.
- Stop after generating local files, filling a draft editor, or opening Preview; report what remains for the user to review manually.
- Do not treat a user request to “发出去” as permission to bypass this Skill's draft-only boundary.
- Keep all generated content and media local unless the user separately uploads it themselves.
...
| `scripts/x-article.ts` | Long-form article draft preparation (Markdown), CDP fallback |
...
Never use the in-app Browser for X draft workflows.
...
10. Stop after Preview; do not click `Publish`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.leslie-skills/leslie-post-to-x/EXTEND.md` | Project |
| 2 | `${XDG_CONFIG_HOME:-$HOME/.config}/leslie-skills/leslie-post-to-x/EXTEND.md` | XDG |
| 3 | `$HOME/.leslie-skills/leslie-post-to-x/EXTEND.md` | User home |

If none found, use defaults.

**EXTEND.md supports**: Default Chrome profile and `default_mode`. This customized copy requires `default_mode: draft-only`; it is a guardrail reminder and does not authorize any public submission.
...
| Profile dir | Shared profile at `leslie-skills/chrome-profile` (see CLAUDE.md Chrome Profile section) |
...
5. Stop before `Post`; do not click it.
...
3. Type the comment.
4. Stop before `Post`; do not click it.
...
9. Stop after Preview; do not click `Publish`.
...
**Draft-only safety**: Do not use `--submit` and never click `Publish` or `Post` in this customized copy. Stop at the local/browser draft and preview.
