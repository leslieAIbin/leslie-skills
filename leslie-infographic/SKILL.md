...
The checked-in project preferences pin raster rendering to the customized `leslie-image-gen` backend, which uses ZenMux. Keep `ZENMUX_API_KEY` outside the Skill directory.
...
3. **Auto-select** (when the preference is `auto`, unset, or the pinned backend isn't available):
   - **Codex (`imagegen`)** — first, inspect your available-skills / tool inventory. If a skill named `imagegen` is listed, you are running inside Codex and MUST use it: invoke via the `Skill` tool with `skill: "imagegen"`, passing the saved prompt file's content (plus output path and aspect ratio per Codex `imagegen`'s own args). Codex `imagegen` is the official raster backend in that runtime and outranks any non-native skill (e.g., `leslie-image-gen`) unless the user has explicitly pinned a different `preferred_image_backend`.
   - **Codex via `codex exec` (`codex-imagegen`)** — if the current runtime exposes no native `imagegen` skill but the `codex` CLI is on `PATH` with an active `codex login`, route through `leslie-image-gen --provider codex-cli` (preferred), or — if leslie-image-gen is unavailable — invoke the bundled wrapper directly. Details, parameters, and the runtime-discovery procedure live in [references/codex-imagegen.md](references/codex-imagegen.md) — load that file only when this branch is selected.
   ...
   - Otherwise, if exactly one non-native backend is installed (e.g., `leslie-image-gen`), use it.
...
Concrete tool names (`imagegen`, `GenerateImage`, `image_generate`, `leslie-image-gen`) above are examples — substitute the local equivalents under the same rule.
...
- If `usage: direct` AND the chosen backend accepts reference images (e.g., `leslie-image-gen` via `--ref`) → pass the file via the backend's ref parameter
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.leslie-skills/leslie-infographic/EXTEND.md` | Project |
| 2 | `${XDG_CONFIG_HOME:-$HOME/.config}/leslie-skills/leslie-infographic/EXTEND.md` | XDG |
| 3 | `$HOME/.leslie-skills/leslie-infographic/EXTEND.md` | User home |
...
4. Call the chosen backend with the prompt file and output path.
   - **`codex-imagegen` invocation**: when the rule resolves to `codex-imagegen`, see [references/codex-imagegen.md](references/codex-imagegen.md) for the invocation contract (preferred `leslie-image-gen --provider codex-cli` path, runtime wrapper discovery, parameter notes, stdout schema, batch semantics).
...
- **Reconfigure interactively** — delete EXTEND.md (or ask "reconfigure leslie-infographic preferences" / "重新配置"). The next run re-triggers first-time setup.
- **Common one-line edits**:
  - `preferred_image_backend: auto` — default; runtime-native tool wins, falls back to the only installed backend, asks only if multiple non-native are present.
  - `preferred_image_backend: codex-imagegen` — pin to Codex's built-in.
  - `preferred_image_backend: leslie-image-gen` — pin to the leslie-image-gen skill.
